...
public class RaindropApiClientImpl implements RaindropApiClient {
    ...
    public List<RaindropTag> getUserTags(String userId) {
        log.atDebug()
                .setMessage("Fetching tags for user: {}")
                .addArgument(userId)
                .log();

        try {
            TagsResponse response = restClient.get().uri("/tags").retrieve().body(TagsResponse.class);

            if (response == null || response.items == null) {
                log.atWarn()
                        .setMessage("No tags found for user: {}")
                        .addArgument(userId)
                        .log();
                return List.of();
            }
            ...
            log.atError()
                    .setMessage("Failed to fetch tags for user: {}")
                    .addArgument(userId)
                    .setCause(e)
                    .log();
    ...
    public List<RaindropCollection> getUserCollections(String userId) {
        log.atDebug()
                .setMessage("Fetching collections for user: {}")
                .addArgument(userId)
                .log();

        try {
            CollectionsResponse response =
                    restClient.get().uri("/collections").retrieve().body(CollectionsResponse.class);

            if (response == null || response.items == null) {
                log.atWarn()
                        .setMessage("No collections found for user: {}")
                        .addArgument(userId)
                        .log();
                return List.of();
            }
            ...
            log.atInfo()
                    .setMessage("Fetched {} collections for user {}")
                    .addArgument(collections.size())
                    .addArgument(userId)
                    .log();
            return collections;
            ...
            log.atError()
                    .setMessage("Failed to fetch collections for user: {}")
                    .addArgument(userId)
                    .setCause(e)
                    .log();
    ...
    public boolean bookmarkExists(Long collectionId, String url) {
        log.atDebug()
                .setMessage("Checking if bookmark exists in collection {}: {}")
        ...
        try {
            ...
            boolean exists = response != null && response.items != null && !response.items.isEmpty();

            log.atDebug()
                    .setMessage("Bookmark exists in collection {}: {}")
                    .addArgument(collectionId)
                    .addArgument(exists)
                    .log();
            return exists;
            ...
            log.atError()
                    .setMessage("Failed to check bookmark existence: {}")
                    .addArgument(url)
                    .setCause(e)
                    .log();
    ...
    public void createBookmark(Long collectionId, String url, String title, List<String> tags) {
        log.atDebug()
                .setMessage("Creating bookmark in collection {}: {}")
                .addArgument(collectionId)
                .addArgument(url)
                .log();

        try {
            CreateRaindropRequest request = new CreateRaindropRequest(url, title, collectionId, tags);
            ...
            log.atError()
                    .setMessage("Failed to create bookmark: {}")
                    .addArgument(url)
                    .setCause(e)
                    .log();
    ...
    public List<RaindropCollection> getCollections() {
        ...
        try {
            ...
            log.atDebug()
                    .setMessage("Retrieved {} collections")
                    .addArgument(collections.size())
                    .log();
            return collections;
    ...
    public Long createCollection(String title) {
        ...
        try {
            ...
            CollectionCreateResponse response = restClient
                    .post()
                    .uri("/collection")
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(requestBody)
            ...
            if (response == null || response.item == null) {
                log.atError()
                        .setMessage("Failed to create collection '{}': null response")
                        .addArgument(title)
                        .log();
            ...
            log.atInfo()
                    .setMessage("Created collection '{}' with ID: {}")
                    .addArgument(title)
                    .addArgument(collectionId)
                    .log();
            return collectionId;
    ...
    private record TagItem(@JsonProperty("_id") String tag, int count) {}

    private record CollectionsResponse(List<CollectionItem> items) {}

    private record CollectionItem(@JsonProperty("_id") Long id, String title) {}

    private record RaindropsResponse(List<RaindropItem> items) {}

    private record RaindropItem(@JsonProperty("_id") Long id, String link, String title) {}

    private record CreateRaindropRequest(String link, String title, Collection collection, List<String> tags) {
    ...
    private record Collection(@JsonProperty("$id") Long id) {}
