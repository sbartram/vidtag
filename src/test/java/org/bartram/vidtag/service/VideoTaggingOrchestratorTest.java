...
class VideoTaggingOrchestratorTest {
    ...
    void processPlaylist_shouldEmitStartedEvent() {
        ...
        when(raindropService.resolveCollectionId(eq("default"), eq("Tech Videos"))).thenReturn(1001L);
    ...
    void processPlaylist_shouldSkipDuplicateVideos() {
        ...
        when(raindropService.bookmarkExists(eq(1001L), eq("https://youtube.com/watch?v=video1"))).thenReturn(true);
    ...
    void processPlaylist_shouldProcessAndTagVideos() {
        ...
        List<TagWithConfidence> generatedTags = List.of(
            new TagWithConfidence("java", 0.95, true),
            new TagWithConfidence("tutorial", 0.85, false)
        );
        ...
        when(videoTaggingService.generateTags(eq(video), eq(existingTags), any())).thenReturn(generatedTags);
        ...
        verify(raindropService).createBookmark(
            eq(1001L),
    ...
    void processPlaylist_shouldEmitBatchCompletedAfterBatch() {
        ...
        when(raindropService.bookmarkExists(eq(1001L), anyString())).thenReturn(false);
        when(videoTaggingService.generateTags(any(), any(), any())).thenReturn(
            List.of(new TagWithConfidence("test", 0.9, false))
        );
    ...
    void processPlaylist_shouldBuildCorrectSummary() {
        ...
        when(raindropService.bookmarkExists(eq(1001L), eq("https://youtube.com/watch?v=v1"))).thenReturn(true);
        // v2 - successful
        when(raindropService.bookmarkExists(eq(1001L), eq("https://youtube.com/watch?v=v2"))).thenReturn(false);
        when(videoTaggingService.generateTags(eq(videos.get(1)), any(), any()))
            .thenReturn(List.of(new TagWithConfidence("test", 0.9, false)));
        // v3 - fails
        when(raindropService.bookmarkExists(eq(1001L), eq("https://youtube.com/watch?v=v3"))).thenReturn(false);
    ...
    void processPlaylist_shouldEmitProgressEventForFetchingVideos() {
        ...
        assertThat(progressEvents.stream().anyMatch(e -> e.message().contains("Fetching"))).isTrue();
    }
    ...
    void processPlaylist_shouldContinueProcessingAfterIndividualVideoFailure() {
        ...
        verify(raindropService).createBookmark(eq(1001L), eq("https://youtube.com/watch?v=v2"), eq("Video 2"), anyList());

        ProcessingSummary summary = (ProcessingSummary) capturedEvents.get(capturedEvents.size() - 1).data();
        assertThat(summary.succeeded()).isEqualTo(1);
        assertThat(summary.failed()).isEqualTo(1);
    }
